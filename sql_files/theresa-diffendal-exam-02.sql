...
select if(district_state = "US" or district_state = "00" or district_state = "DC" or district_state is null, "national-level", district_state) as location, sum(expend_amt), count(*)
from exercises.outside1
...
#' for Clinton' = $250,312,813.06	'for Trump' = $205,245,220.63	'other' = $4,433,817.47 	so greater for Clinton
select 
...
# 536 contributions by lawyers or attorneys to HC
select *
from exercises.md_contrib t join exercises.md_cand d on t.cmte_id=d.cmte_id
...
#16
select t.amount, d.cand_name
from exercises.md_contrib t join exercises.md_cand d on t.cmte_id=d.cmte_id
where cand_name like "%Cruz%"
and t.amount < 0;
...
#Jill Stein was only candidate with less with $333.33 average donation amount
select d.cand_name, avg(t.amount)
from exercises.md_contrib t join exercises.md_cand d on t.cmte_id=d.cmte_id
where t.amount > 0
group by d.cand_name
